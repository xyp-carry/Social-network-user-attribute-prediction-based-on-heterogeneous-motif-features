...
import copy
import numpy as np
import find_motif_1 as fm

"""这一版有错误在对称破坏的地方"""


def classification_motif(MS, SL, deep, deeps, excel, Motif_S, do_info, info, test, motif_numb):
    if Motif_S == 'M4_1' or Motif_S == 'M4_11' or Motif_S == 'M4_9':
        return fm.Search_M1_M4_M6(MS, SL, deep, deeps, excel, Motif_S, do_info, info, test, motif_numb)
    elif Motif_S == 'M4_2' or Motif_S == 'M4_8' or Motif_S == 'M4_10' or Motif_S == 'M4_5' or Motif_S == 'M4_7':
        return fm.Search_M1_1_M3_M4_1_M5_M3_2(MS, SL, deep, deeps, excel, Motif_S, do_info, info, test, motif_numb)
    elif Motif_S == 'M4_4' or Motif_S == 'M4_3' or Motif_S == 'M4_6':
        return fm.Search_M2_M2_1_M3_1(MS, SL, deep, deeps, excel, Motif_S, do_info, info, test, motif_numb)
    elif Motif_S == 'M3_3' or Motif_S == 'M3_2':
        return fm.Search_M33_1_M33_2(MS, SL, deep, deeps, excel, Motif_S, do_info, info, test, motif_numb)
    elif Motif_S == 'M3_1':
        return fm.Search_M33_2_1(MS, SL, deep, deeps, excel, Motif_S, do_info, info, test, motif_numb)
...
def find_motif(G, Motif, G_node_list, G2_node_list, n_G2, MS, deeps, derepeat, weighted=False, excel=None, Motif_S=None,
               info=None, Classification=False, test=None, motif_numb=None):
    # start = time.time()
    number = 0  # 存储模体数量
    deep = deeps
    SL = []
    do_info = [3 for x in range(n_G2)]  # 记录节点具备的信息
    if len(do_info) == 3:
        if Classification == True:
            do_info[0] = info._get_value(MS[0] - 1, 'info')
    ...
    for i in range(deeps):
        ss = []
        if not isinstance(MS[i], np.int64):
            MS[i] = int(MS[i])
    ...
    if n_G2 - deeps < 2:
        if motif_numb != None:
            classification_motif(MS, SL, deep, deeps, excel, Motif_S, do_info, info, test, motif_numb)
        return len(SL[0][0])
    Quelist = []
    ...
    if Classification == True:
        for i in range(deep):
            do_info[i] = info._get_value(MS[i] - 1, 'info')
    while (1):
        if deep == n_G2 - 1:
            number += len(SL[deep - deeps][0])
            # for i in SL[deep-deeps][0]:
            #     print(MS[0],MS[1],MS[2],i)
            if motif_numb != None:
                classification_motif(MS, SL, deep, deeps, excel, Motif_S, do_info, info, test, motif_numb)
            deep -= 1
            do_info[deep] = 'x'
        ...
        if derepeat[deep] != -1:
            ...
            while (int(MS[deep]) < int(MS[derepeat[deep]])):
                ...
                if derepeat[deep] == -1:
                    break
            if Classification == True:
                do_info[deep] = info._get_value(MS[deep] - 1, 'info')
        else:
            MS[deep] = Quelist[deep - deeps].pop()
            if test != None:
                while (not Quelist[deep - deeps]):
                    if (MS[deep - 1], MS[deep]) in test or (MS[deep], MS[deep - 1]) in test:
                        MS[deep] = Quelist[deep - deeps].pop()
                    else:
                        break
                while (not Quelist[deep - deeps]):
        ...
        for i in range(1, n_G2 - deep):
            if deep == 1 and test != None:
                if (MS[0], MS[1]) not in test and (MS[1], MS[0]) not in test:
                    break
            ...
            if weighted:
                for k in searchset:
                    if (not Feasibility_weighted(G, Motif, G2_node_list[deep], G2_node_list[deep + i], MS[deep], k,
                                                 hasEdge)):
                        ss.append(k)
...
def find_motif_directed(G, Motif, G_node_list, G2_node_list, n_G2, MS, deeps, derepeat, weighted=False):
    ...
    for i in range(deeps):
        ...
        for j in range(deeps, n_G2):
            ...
            if weighted:
                for k in searchset1:
                    if (not Feasibility_weighted_directed(G, Motif, G2_node_list[i], G2_node_list[j], MS[i], k, hasEdge,
                                                          hasEdge1)):
                        ss.append(k)
            searchset1 = searchset1 - set(ss)
            if (not searchset1):
    ...
    while (1):
        ...
        for i in range(1, n_G2 - deep):
            ...
            if weighted:
                for k in searchset:
                    if (
                    not Feasibility_weighted_directed(G, Motif, G2_node_list[deep], G2_node_list[deep + i], MS[deep], k,
                                                      hasEdge, hasEdge1)):
                        ss.append(k)
            searchset = searchset - set(ss)
            if (not searchset):
...
def node_motif_num(G, G_motif, node, motif_num=None, Motif_S=None, info=None, directed=False, weighted=False):
    """
    ...
    if directed:
        ...
        # 下面查找node节点的模体数
        MS[0] = node
        node_motif_number = find_motif(G, G_motif, G_node_list, G2_node_list, len1, MS, 1, derepeat, weighted, None,
                                       Motif_S, info, True, None, motif_num)
        MS[0] = G2_node_list[0]
        a = find_motif(G_motif, G_motif, G2_node_list, G2_node_list, len1, MS, 1, derepeat, weighted)
        for i in motif_num:
            if a != 0:
                motif_num[i] = motif_num[i] / a
    # end = time.time()
...
def edge_motif_num(G, G_motif, edge, motif_num=None, Motif_S=None, info=None, directed=False, weighted=False):
    """
        计算node节点作为模体的第一个节点所参与的模体数量
        :param G: 图或（网络），可以是有向或无向图，是networkx.Graph() 或 networkx.DiGraph()
        :param G_motif: 模体图,可以有向或者无向但必须与图G一致
    ...
    if directed:
        ...
        if twoWay:  # 如果是双向边还需要查找反方向的模体数
            ...
            MS[1] = G2_node_list[0]
            repetitions.extend(
                find_motif_directed_list(G_motif, G_motif, G2_node_list, G2_node_list, len1, MS, 2, weighted))
        derepeat = get_prevent_repetition_list(G_motif, G2_node_list, len1, repetitions)
        # 计算边模体数量01的顺序不同
        MS[0] = edge[0]
        MS[1] = edge[1]
        ...
        if twoWay:  # 如果是双向边还需要查找反方向的模体数
            MS[0] = edge[1]
            MS[1] = edge[0]
            edge_motif_number += find_motif_directed(G, G_motif, G_node_list, G2_node_list, len1, MS, 2, derepeat,
                                                     weighted)
            MS[0] = G2_node_list[1]
            MS[1] = G2_node_list[0]
            a += find_motif_directed(G_motif, G_motif, G2_node_list, G2_node_list, len1, MS, 2, derepeat, weighted)
        ...
        test_list.append(repetitions[0])
        derepeat = get_prevent_repetition_list(G_motif, G2_node_list, len1, test_list)
        # ------------------------------#
        MS[0] = edge[0]
        MS[1] = edge[1]
        edge_motif_number = find_motif(G, G_motif, G_node_list, G2_node_list, len1, MS, 2, derepeat, weighted, None,
                                       Motif_S, info, True, None, motif_num)
        MS[0] = G2_node_list[0]
        MS[1] = G2_node_list[1]
        ...
        if twoWay:  # 如果是双向边还需要查找反方向的模体数
            ...
            edge_motif_number += find_motif(G, G_motif, G_node_list, G2_node_list, len1, MS, 2, derepeat, weighted,
                                            None, Motif_S, info, True, None, motif_num)
            MS[0] = G2_node_list[1]
            MS[1] = G2_node_list[0]
            a += find_motif(G_motif, G_motif, G2_node_list, G2_node_list, len1, MS, 2, derepeat, weighted)
        for i in motif_num:
        ...
        for i in motif_num:
            if change_5 == 1:
                motif_num[i] = motif_num[i] - 0.5
                change_5 = 0
            if motif_num[i] * 10 % 10 != 0:
                motif_num[i] = motif_num[i] + 0.5
                change_5 = 1
    # end = time.time()
...
def total_motif_num(G, G_motif, excel=None, Motif_S=None, info=None, directed=False, weighted=False, test=None):
    """
        计算模体总数
        :param G: 图或（网络），可以是有向或无向图，是networkx.Graph() 或 networkx.DiGraph()
    ...
    if directed:
        ...
        derepeat = get_prevent_repetition_list(G_motif, G2_node_list, n_G2, repetitions)
        total_motif_number = find_motif(G, G_motif, G_node_list, G2_node_list, n_G2, MS, 0, derepeat, weighted, excel,
                                        Motif_S, info, True, test)
...
if __name__ == "__main__":
    ...
    G = nx.read_edgelist("Caltech36.txt", create_using=nx.DiGraph())
    G = G.to_undirected()
    # G_motif = nx.read_edgelist("mydata/motif/directed/motif4_3_0.txt", create_using=nx.DiGraph())
    g = nx.DiGraph()
    g.add_nodes_from([1, 2, 3, 4])
    ...
    number2 = 0
    start3 = time.time()
    print(node_motif_num(G, g, "129", directed=False, weighted=False))
