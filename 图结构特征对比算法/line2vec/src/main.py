...
def parse_args():
    ...
    parser.add_argument('--output', nargs='?', default='emb/Caltech36.emb',
                        help='Embeddings path')

    parser.add_argument('--line-graph', nargs='?', default='graph/karate_line.edgelist',
                        help='Line graph path')

    parser.add_argument('--dimensions', type=int, default=128,
                        help='Number of dimensions. Default is 128.')

    parser.add_argument('--walklength', type=int, default=80,
...
def learn_embeddings(walks, edge_map, reverse_edge_map, nodes, neighbors):
    ...
    for i in range(1):#args.l2v_iter
...
def map_edge_to_unique_index(G):
    ...
    # base_path = os.path.dirname("Bowdoin47.txt")
    # print(base_path)
...
def load_edge_map():
    base_path = os.path.dirname("Caltech36.txt")
    edge_to_node_id_dict_filename = os.path.join(base_path, 'edge_map.pkl')
...
def main(args):
    ...
    nx_G = read_graph()
    nx_G =nx.read_edgelist('Caltech36.txt', nodetype=int)
    print("Number of nodes in the original graph : ", len(nx_G.nodes()))
    print("Number of edges in the original graph : ", len(nx_G.edges()))

    # edges = len(nx_G.edges())
    # c = 0
    # for edg in nx_G.edges():
    #     nx_G[edg[0]][edg[1]]['weight'] = 1


    # while(1):
    #     try:
    #         nx_G.add_weighted_edges_from([(random.choice(nx_G.nodes()),random.choice(nx_G.nodes()),0)])
    #         c = c + 1
    #     except:
    #         continue
    #     if c == edges:
    ...
    edg_game = {}
    # for edg in nx_G.edges():
    #     edg_game[edg]=nx_G[edg[0]][edg[1]]['weight']
    # import pickle
    # with open("myDictionary.pkl", "wb") as tf:
    #     pickle.dump(edg_game, tf)
    # while(1):
    #     1
    if 1:
    ...
    nx_L = nx.read_edgelist('Caltech36.txt', nodetype=int)
    nx_L = nx_L.to_undirected()
    # L = node2vec.Graph(nx_L, args.directed, args.p, args.q)
    L = node2vec.Graph(nx_L, False, 0.5, 0.5)
    #init Node2vec model

    print ('begin')
    L.preprocess_transition_probs()
    walks = L.simulate_walks(10, 80)
