...
def your_harmonic_function(Graph, label_name):
    ...
    print([value for value in labeled_list])
    print(uniq_labeles_inverse)
    return [uniq_labeles_inverse[int(round(value))] for value in labeled_list]


# G = create_toroidal(16)
# G.nodes[0]['label'] = 'blue'
# G.nodes[255]['label'] = 'red'
...
df = get_info()
print(df)
node_color = list(df.iloc[0:8277,1])
print(node_color)
...
for n in G.nodes:
    print(type(n))
    if int(n) < 8277:
        G.nodes[n]['label'] = node_color[int(n)]
